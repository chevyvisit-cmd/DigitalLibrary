      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-4 w-80 glass-card border border-white/10 shadow-2xl overflow-hidden z-[60]"
          >
            ...
            <div className="max-h-[350px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10">
              {notifications.length > 0 ? (
                notifications.map((notif) => (
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <h4 className="text-sm font-bold text-white/90">{notif.title}</h4>
                        <p className="text-xs text-white/40 mt-1 leading-relaxed">{notif.message}</p>
                      </div>
                      <button 
                        onClick={() => markAsRead(notif.id)}
                        className="p-1.5 bg-white/5 rounded-lg text-white/20 hover:text-primary hover:bg-primary/10 transition-all opacity-0 group-hover:opacity-100"
                ...
                <div className="p-10 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm text-white/40 font-medium">Hozircha bildirishnomalar yo'q</p>
                </div>
              )}
            </div>

            {notifications.length > 0 && (
              <button className="w-full p-3 text-[11px] font-bold text-white/40 hover:text-white transition-colors uppercase tracking-widest border-t border-white/5 hover:bg-white/5">
                Barchasini ko'rish
              </button>
            )}
          </motion.div>
